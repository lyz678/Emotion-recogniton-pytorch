...
file_model = 'ResNet18.sim.om'	#'minXception.sim.om'  'ResNet18.sim.om'

model = InferSession(0, file_model)
...
while True:
    ...
    if len(faces) > 0:
        ...
        # 模型推理
        preds = model.infer([roi])[0]

        prob = np.max(preds)
        label = EMOTIONS[preds.argmax()]
        text = label + " {:.2f}%".format(prob * 100)
        
        cv2.putText(frameClone, text, (fX, fY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
